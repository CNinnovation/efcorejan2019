...
namespace FirstSample.Services
{
    public class MyBooksController : IMyBooksController
    {
        ...
        {
            var tx = _booksContext.Database.BeginTransaction();


           
            var b3 = new Book { Title = "Programming UWP Apps", Publisher = "Self" };
            _booksContext.Books.Add(b3);

            _booksContext.SaveChanges();

            // _booksContext.ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking;
            var b1 = _booksContext.Books.FirstOrDefault(b => b.Publisher.StartsWith("Wrox"));

            var b2 = _booksContext.Books.FirstOrDefault(b => b.Title.StartsWith("Pro"));
            ...
            b1.Title = "Professional C# 8";

            var entries = _booksContext.ChangeTracker.Entries<Book>();
            ...
            _booksContext.Database.CommitTransaction();
        }
    }
}
